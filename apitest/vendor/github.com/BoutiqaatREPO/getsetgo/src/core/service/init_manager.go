...
import (
	...
	"github.com/BoutiqaatREPO/getsetgo/src/common/logger/auditlog"
	...
	"github.com/BoutiqaatREPO/getsetgo/src/common/profiler"
	...
	"github.com/BoutiqaatREPO/getsetgo/src/core/common/utils/responseheaders"
...
func InitVersionManager() {
	...
	service := versionmanager.BasicVersion{
		Resource: "SERVICE",
		Version:  "V1",
		Action:   "GET",
	...
	healtchCheckService := versionmanager.BasicVersion{
	...
	vmap := versionmanager.VersionMap{
		service: serviceParam, healtchCheckService: healthCheckParam,
	}

	//Disable blackbox logs for healthcheck api.
	blackbox.BlackBoxAPIDisableMap[healtchCheckService] = true
...
func addAPIVersions(vmap versionmanager.VersionMap) {
	for _, apiInstance := range apiList {
		...
		if customapi, ok := apiInstance.(APIInterfaceCustom); ok {
			//Add Settings for BlackBox Logs
			b := versionmanager.BasicVersion{
				Resource: version.Resource,
				Action:   version.Action,
				Version:  version.Version,
				BucketID: version.BucketID,
			}
			blackbox.BlackBoxAPIDisableMap[b] = customapi.GetOptions().DisableBlackBoxLogs
			audit.AuditAPIDisableMap[b] = customapi.GetOptions().DisableAuditLogs
		}
...
func InitMonitor() {
	if err := monitor.InitializeDefault(&config.GlobalAppConfig.MonitorConfig); err != nil {
